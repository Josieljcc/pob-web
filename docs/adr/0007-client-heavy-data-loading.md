...
**Versioned static asset:** the SPA loads `GET /tree-data/<version>/tree.json` (see `passiveTreeJsonAssetPath` in `@pob-web/domain`); the repo ships a **minimal** file under `apps/web/public/tree-data/` for dev/tests. Replace with a full export from your pipeline without blocking other phases.
